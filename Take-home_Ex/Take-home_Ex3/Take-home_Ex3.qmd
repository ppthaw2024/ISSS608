...
summarised_temp_week_ts <- summarised_temp_week %>%
  as_tsibble(index = week, key = Region) %>%
  fill_gaps()

weekly_region_temp <- summarised_temp_week_ts %>%
  filter(Region == "North")


gg_tsdisplay(weekly_region_temp, avg_temp, plot_type = "partial") +
  labs(title = "Weekly Mean Temperature (°C) - North Region")
